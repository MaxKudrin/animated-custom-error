...
WebFontConfig = {
  ...
  function FiveOhFiveFont(ctx, canvas) {
    FiveOhFiveContext.fillText("12345", 275, 100, 550);
  ...
  $('.left').click(function() {    
    ...
    $('.Wrapper').append($child);
      $child.css({ top:X, left: -100+Y })
        .animate({ top: nTop+'px', left: nLeft+'px' }, 8000)}
  });
  ...
  $('.right').click(function() {    
    for (var j = 1; j <= 700; j++){        
      ...
      var Y = (Math.random()*WrapperH)%(WrapperH>>0);
      var nTop = Math.floor((Math.random()*WrapperW));
      var nLeft = Math.floor(((Math.random()*WrapperW)));
      var $child = $(this).clone();       
  
      $('.Wrapper').append($child);
      $child.css({ top:X, left: 270+Y })
        .animate({ top: nTop+'px', left:200+nLeft+'px' }, 8000)}                   
  });
